...
//  Pulled from https://github.com/Raphcal/some-corelibs-port/blob/c6bf41c903bc5c23eec22d9eb6de56c5c8b3676c/keyboard/src/keyboard.c
...
static AudioSample * _Nonnull newSampleOrError(const char * _Nonnull path) {
    ...
    if (sample == NULL) {
        pd->system->error("Unable to load sample at path: %s", path);
    }
    return sample;
}
...
static void playSound(SamplePlayer * _Nonnull * _Nullable samplePlayer, AudioSample * _Nonnull * _Nullable sample, const char * _Nonnull path) {
    ...
    if (theSamplePlayer == NULL) {
        theSamplePlayer = pd->sound->sampleplayer->newPlayer();
        *samplePlayer = theSamplePlayer;
    }
    pd->sound->sampleplayer->setSample(theSamplePlayer, theSample);
    pd->sound->sampleplayer->play(theSamplePlayer, 1, 1.0f);
}

static void freeSounds(PDKeyboard * _Nonnull self) {
    if (self->samplePlayer) {
        pd->sound->sampleplayer->freePlayer(self->samplePlayer);
        self->samplePlayer = NULL;
    }
    if (self->columnNextSound) {
        pd->sound->sample->freeSample(self->columnNextSound);
        self->columnNextSound = NULL;
    }
    if (self->columnPreviousSound) {
        pd->sound->sample->freeSample(self->columnPreviousSound);
        self->columnPreviousSound = NULL;
    }
    if (self->rowSound) {
        pd->sound->sample->freeSample(self->rowSound);
        self->rowSound = NULL;
    }
    if (self->bumpSound) {
        pd->sound->sample->freeSample(self->bumpSound);
        self->bumpSound = NULL;
    }
    if (self->keySound) {
        pd->sound->sample->freeSample(self->keySound);
        self->keySound = NULL;
    }
}
...
static void fillRoundRect(struct rectangle rectangle, LCDColor color) {
    if (rectangle.size.width < 4 || rectangle.size.height < 4) {
        pd->graphics->fillRect(rectangle.origin.x, rectangle.origin.y, rectangle.size.width, rectangle.size.height, color);
        return;
    }
    pd->graphics->drawLine(rectangle.origin.x + 2, rectangle.origin.y,
                                 rectangle.origin.x + rectangle.size.width - 3, rectangle.origin.y,
                                 1, color);
    pd->graphics->drawLine(rectangle.origin.x + 1, rectangle.origin.y + 1,
                                 rectangle.origin.x + rectangle.size.width - 2, rectangle.origin.y + 1,
                                 1, color);
    pd->graphics->fillRect(rectangle.origin.x, rectangle.origin.y + 2, rectangle.size.width, rectangle.size.height - 4, color);
    pd->graphics->drawLine(rectangle.origin.x + 1, rectangle.origin.y + rectangle.size.height - 2,
                                 rectangle.origin.x + rectangle.size.width - 2, rectangle.origin.y + rectangle.size.height - 2,
                                 1, color);
    pd->graphics->drawLine(rectangle.origin.x + 2, rectangle.origin.y + rectangle.size.height - 1,
...
static void drawKeyboard(PDKeyboard * _Nonnull self) {
    ...
    pd->graphics->setFont(keyboardFont);
    for (unsigned int index = 0; index < kColumnMenu; index++) {
        ...
        pd->graphics->drawText(&glyph, 1, kASCIIEncoding, cx, cy);
...
static void PDKeyboardMutableTextGrow(PDKeyboardMutableText * _Nonnull self, int newSize) {
    self->super.data = pd->system->realloc(self->super.data, newSize * sizeof(char));
...
static void handleMenuCommand(PDKeyboard * _Nonnull self) {
    ...
    switch (selectedMenuOption) {
        ...
        case kMenuOptionCancel:
            ...
            break;
        default:
            pd->system->error("Unsupported menu option: %d", selectedMenuOption);
            break;
    }
}
...
static void enterNewLetterIfNecessary(PDKeyboard * _Nonnull self) {
    ...
    const unsigned int currentMillis = pd->system->getCurrentTimeMilliseconds();
    
    PDButtons pressing;
    PDButtons justPressed;
    pd->system->getButtonState(&pressing, &justPressed, NULL);
...
static void updateAnimation(PDKeyboard * _Nonnull self) {
    const unsigned int animationTime = pd->system->getCurrentTimeMilliseconds() - self->animationStartTime;
    ...
    if (animationTime >= self->animationDuration) {
        ...
        switch (self->currentAnimationType) {
            ...
            case kAnimationTypeKeyboardHide:
                ...
                pd->system->setUpdateCallback(self->playdateUpdate, self->playdateUpdateUserdata);
...
static void startAnimation(PDKeyboard * _Nonnull self, PDKeyboardAnimationType animationType, unsigned int duration) {
    ...
    if (self->currentAnimationType != kAnimationTypeNone) {
        self->animationDuration = pd->system->getCurrentTimeMilliseconds() - self->animationStartTime;
        updateAnimation(self);
    }
    
    self->animationDuration = duration;
    self->currentAnimationType = animationType;
    self->animationStartTime = pd->system->getCurrentTimeMilliseconds();
}
...
static void moveSelectionUp(PDKeyboard * _Nonnull self, int count, bool_t shiftRow) {
    ...
    self->animationStartTime = pd->system->getCurrentTimeMilliseconds() - (1000 / self->refreshRate);
...
static void checkButtonInputs(PDKeyboard * _Nonnull self) {
    ...
    pd->system->getButtonState(&pressing, &justPressed, &justReleased);
...
// override on the main pd.update function so that we can run our animations without requiring timers
static int keyboardUpdate(void * _Nonnull userdata) {
    ...
    if (self->isVisible) {
        ...
        if (!self->justOpened) {
            ...
            const float crankChange = pd->system->getCrankChange();
            if (crankChange != 0.0f) {
...
static LCDBitmap * _Nonnull loadBitmapOrError(const char * _Nonnull path) {
    ...
    if (error) {
        pd->system->error("Unable to load bitmap at path %s, error: %s", path, error);
    }
    return bitmap;
}
...
static void loadFontAndImages(void) {
    ...
    fontHeight = pd->graphics->getFontHeight(keyboardFont);
...
static PDKeyboard * _Nonnull PDKeyboardNew(void) {
    PDKeyboard *self = pd->system->realloc(NULL, sizeof(PDKeyboard));
    ...
    *self = (PDKeyboard) {
        ...
        // TODO: Ideally, would be retrieved with pd->display->getRefreshRate()
        .refreshRate = 30.0f,
...
static void PDKeyboardShow(PDKeyboard * _Nonnull self, const char * _Nullable newText, const unsigned int newTextLength) {
    if (self->playdateUpdate == NULL) {
        pd->system->error("playdateUpdate must be defined before calling show()");
        return;
    }
    ...
    self->originalText.data = pd->system->realloc(self->originalText.data, newTextLength + 1);
    ...
    pd->system->setUpdateCallback(keyboardUpdate, self);
...
static void PDKeyboardGetText(PDKeyboard * _Nonnull self, char * _Nonnull * _Nullable text, unsigned int * _Nullable count) {
    ...
    char *data = pd->system->realloc(*text, (charCount + 1) * sizeof(char));
...
static void PDKeyboardSetCapitalizationBehavior(PDKeyboard * _Nonnull self, PDKeyboardCapitalization behavior) {
    if (behavior < kCapitalizationNormal || behavior > kCapitalizationSentences) {
        pd->system->error("Please use one of the following options: pd.keyboard.kCapitalizationNormal, pd.keyboard.kCapitalizationWords, pd.keyboard.kCapitalizationSentences");
    }
